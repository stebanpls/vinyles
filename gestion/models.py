    ...
    def save(self, *args, **kwargs):
        ...
        if self.foto_perfil and hasattr(self.foto_perfil, "path") and "default" not in self.foto_perfil.name:
            try:
                ...
                logger.info(
                    f"Imagen de perfil procesada y guardada para {self.user.username} en {self.foto_perfil.path}"
                )
            except FileNotFoundError:
                logger.error(
                    f"¡Error crítico! El archivo {self.foto_perfil.path} no se encontró para procesar después de guardar."
                )
            except Exception as e:
                logger.error(
                    f"Error procesando imagen de perfil para {self.user.username} en {self.foto_perfil.path}: {e}"
                )
